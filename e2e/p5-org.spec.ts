...
test("組織とプロジェクトを作り、タスクと予算とおぼえ書きを整える", async ({
  browser,
}) => {
  ...
  await page.getByLabel(/つとめ先/).check();
  await page.getByLabel("つどいの名前").fill(orgName);
  ...
  await page.getByPlaceholder("しごとの名前").fill(projectName);
  ...
  await page.getByLabel("なにをするか").fill(taskTitle);
  ...
  await page.getByRole("button", { name: "しまう" }).click();
  ...
  await page.getByRole("button", { name: "のこす" }).click();
...
test("組織のなかまでも、プロジェクトに入るまでは中身が見えない", async ({
  browser,
}) => {
  const context = await browser.newContext();
  const page = await context.newPage();
...
test("プロジェクトに加えられると、タスクの担い手として動ける", async ({
  browser,
}) => {
  ...
  await expect(page.getByRole("button", { name: "とりくみ中へ" })).toHaveCount(
    0,
  );

  // 自分のタスクを作れば動かせる
  await page.getByLabel("なにをするか").fill(`図案おこし ${runId}`);
  await page.getByRole("button", { name: "書き出す" }).click();
  await page.waitForURL(`${projectUrl}/tasks`);
  await page
    .getByRole("button", { name: "とりくみ中へ" })
    .first()
    .click();
  await page.waitForURL(`${projectUrl}/tasks`);
  const doingSection = page
    .locator("section")
    .filter({ hasText: "とりくみ中" })
    .first();
