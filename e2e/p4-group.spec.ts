...
test("装飾つきの日記を書き、写真をしまい、グループへ差し出す", async ({
  browser,
}) => {
  ...
  // 写真をしまう(クライアント圧縮→Storage→photoアイテム)F-04-1
...
test("なかまはアルバムで写真と日記を見られる", async ({ browser }) => {
  const context = await browser.newContext();
  const page = await context.newPage();
  ...
  // 写真(署名付きURL)と日記の綴じ込みが見える(F-07-2)
  await expect(page.locator("img").first()).toBeVisible({ timeout: 15000 });
