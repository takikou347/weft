...
|---|---|---|---|---|---|
| createItem | type, occurred_on, title, body, type別項目, link_to | typeは4種(diary/event/expense/task)のみ / 日付は正規化(不正→今日) / expense以外: 題か本文必須 / expense: 金額は1以上の整数 | ①個人スペースid取得 ②items INSERT(origin=個人スペース=デフォルト非公開) ③link_toがあればlinks INSERT(F-03-4/F-09-3 自動リンク。a<b正規化) | 〔状態返却〕「題か本文のどちらかを記してください。」「金額は1円以上の整数で入れてください。」等 | redirect `/days/{occurred_on}` |
...
| 失敗時 | | 「しまえませんでした。時間をおいてお試しください。」を表示、input をリセット |
...
| createInvitation | space_id | − | invitations INSERT(token/期限7日はDB既定) | ログのみ | redirect なかま画面 |
...
| removeMember | space_id, user_id | − | space_members DELETE(RLS=owner/adminがowner以外を) | ログのみ | redirect なかま画面 |
...
| createProjectTask | 題1〜100、期限正規化 | items INSERT(origin=project, payload.status=todo, assignee任意)→**同時にitem_shares INSERT**(作成=共有) | プロジェクトメンバー |
| changeTaskStatus | − | RPC update_task_status | 作成者+担当者のみ(statusのみ変更可) |
| createProjectDoc | 題1〜100+本文必須 | items(type=document) INSERT+共有 | プロジェクトメンバー |
...
| アクション | 検証 | 処理 |
|---|---|---|
| addSettlement | 題1〜100 / 金額1以上整数 / 対象1名以上 / **支払者・対象全員がスペースメンバーであることをアプリ側でも確認** | settlements INSERT → スペース全員へ通知トリガ。失敗は `?error=1` で文言表示 |
...
| updateProfile | 表示名1〜30文字 → profiles UPDATE | 成功時は「あらためました。」を表示(遷移なし) |
| deleteAccount | 確認語「とじる」一致必須 → ①Storageの自分のフォルダ削除 ②**service_role**で auth.admin.deleteUser(FKカスケードで全行削除) ③signOut | service_roleはこのアクション専用(server-only)。失敗時〔状態返却〕 |
