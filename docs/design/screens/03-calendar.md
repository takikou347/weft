# 03. こよみ(カレンダー)

- URL: `/calendar?view=month|week|list&month=YYYY-MM&date=YYYY-MM-DD&hide=<spaceId,...>`
- アクセス: 要ログイン / 対応項番: F-03-1, F-03-3, F-03-5

自分の記録+**参加スペースへ共有された記録**が重なって表示される(RLSがそのまま可視範囲)。

## 3-1. 月表示(既定)

| レイヤー全表示 | 「山歩きの会」レイヤーOFF | 空(新規ユーザー) |
|---|---|---|
| ![month](../images/calendar-month.png) | ![layer-off](../images/calendar-month-layer-off.png) | ![empty](../images/calendar-empty.png) |

### 画面項目

| No | 項目 | 内容・表示条件 |
|---|---|---|
| 1 | 見出し「YYYY年M月」 | 常時 |
| 2 | 表示切替タブ(月/週/一覧) | 常時。選択中は藍地白文字 |
| 3 | まえの月 / 今月 / つぎの月 | 常時 |
| 4 | レイヤーチップ(F-03-3) | **参加スペースが1つ以上あるとき**。「わたし」+各スペース名(スペースのしるし色の枠)。OFF中は灰色+取り消し線。タップでON/OFFトグル(`hide=`に追加/削除) |
| 5 | 月グリッド | 月曜はじまり。前後月セルは生成り地。**今日**は日番号が藍丸白抜き |
| 6 | 記録ドット(F-03-5) | その日に記録がある種別ぶん表示。●藍=予定 / ●墨=日記 / ●薄墨=収支 / ○=つとめ / **◆(45度回転・スペース色)=なかまの共有** |
| 7 | 凡例 | 常時(◆はスペース参加時のみ) |

### 処理

| 操作 | 遷移・処理 |
|---|---|
| 日付セル | `/days/{date}`(その日ページ) |
| レイヤーチップ | 同画面(hideパラメータをトグル)。OFFにしたレイヤーのドットが消える |
| 月移動 | `?month=` を±1ヶ月 |

## 3-2. 週表示

![week](../images/calendar-week.png)

| No | 項目 | 内容・表示条件 |
|---|---|---|
| 1 | 見出し「M月D日 からの週」 | 週の月曜 |
| 2 | まえの週 / 今週 / つぎの週 | 常時 |
| 3 | 7日分のリスト | 各日: 日付リンク(今日は藍字)→その日ページ。配下にその日の記録(種別ラベル+一行表記)→詳細へ |

一行表記: 予定=「HH:MM 題」(終日は題のみ)/ 収支=「摘要 −1,234円」/ その他=題または本文冒頭。

## 3-3. 一覧表示

![list](../images/calendar-list.png)

| No | 項目 | 内容・表示条件 |
|---|---|---|
| 1 | 見出し「YYYY年M月 の記録」 | |
| 2 | まえの月 / つぎの月 | |
| 3 | 記録リスト | 月内全記録を日付昇順。1行=日付(曜日)+種別+一行表記 → `/items/{id}` |
| 4 | 空状態 | 0件時「この月の記録はまだありません。」 |

## パターン

| パターン | 挙動 |
|---|---|
| month/date が不正値 | 今月/今日にフォールバック(エラーにしない) |
| スペース未参加 | レイヤーチップ・◆凡例は非表示(自分のドットのみ) |
| 共有記録の帰属 | 他人の記録は共有元スペースのレイヤー(複数共有時は最初の1つ) |
| レイヤーOFFの保存 | URLパラメータのみ(共有・ブックマーク可能。永続化はしない) |
