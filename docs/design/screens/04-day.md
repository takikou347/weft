...
| 2 | まえの日 / 月へ戻る / つぎの日 | 常時。月へ戻る→その日の月の月表示 |
| 3 | 作成導線4つ | 常時:「日記を書く」「予定を追加する」「収支を記録する」「タスクを追加する」→ `/items/new?type=◯&date=この日` |
| 4 | 種別セクション | **その種別の記録があるときのみ**。順序: 予定→日記→収支→タスク(→そのほか=文書・写真)。見出しは藍の縦帯 |
| 5 | 記録行 | 一行表記(+日記は本文2行プレビュー)→ `/items/{id}` |
| 6 | 空状態 | 記録0件時:「この日の記録はまだありません。」 |
